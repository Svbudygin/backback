...
async def assept_from_device(amount):
    headers = {
        'accept': 'application/json',
        'Content-Type': 'application/json',
    }
    ...
    json_data = {
        ...
        'message': f'Зачисление {amount}р',
        'device_hash': 'string',
        'bank': 'string',
    }
    
    async with aiohttp.ClientSession() as session:
        response = await session.put('http://127.0.0.1:8000/external-transaction/accept-from-device', headers=headers,
                                     json=json_data)
        try:
            data = await response.json()
        except Exception as e:
...
async def main():
    ...
    l = timer
    for i in range(1):
         asyncio.ensure_future(asyncio.gather(*[create() for _ in range(MAX)]))
    ...
    asyncio.ensure_future(asyncio.gather(*[assept_from_device(i) for i in arr[:MAX]]))
    print(datetime.datetime.now() - timer)
    global iteration
    iteration += 1
    print('TOOOOOOOOOOOOTAL', iteration, datetime.datetime.now() - l)


async def hardcode_test():
    cnt = 1
    while True:
